...
df <- readxl::read_xlsx("C:/Users/User/Desktop/Winter19/Apples/ChoiceAssay2017.xlsx")
##Group df by Plate
...
png("choiceassay.png", width = 15.5, height = 18, units = "cm", res = 500)

#Make the plot
p <- ggplot(df1, aes(x = both, y = mean, ymin = lb, ymax = hb, color = loc)) + 
  geom_bar(stat = "identity", fill = "white", size = 0.8, show.legend = FALSE, position = position_dodge(width = 0.5)) + 
  geom_errorbar(size = 0.8, width = 0.45) + 
  scale_color_manual(name = "Location",
                     values = c("#003300", "#990000", "#003399"),
                     limits = c("lawn", "apple", "agar"),
                     labels = c("agar" = "Outside", "apple" = "Scaffold", "lawn" = "Lawn")
                       ) +
